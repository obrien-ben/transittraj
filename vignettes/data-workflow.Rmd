...
``` {r, out.width = "60%", dpi = 800, fig.align = "center", fig.width = 1, fig.height = 2}
...
roughly follow I-295 down to WMATA's
[Shepherd Parkway](https://maps.app.goo.gl/RWNECRptbsDovPBJA) bus garage.
These points most likely correspond to one or more deadheading
vehicle that were logged into a trip but were not actually in revenue service.
`transittraj` includes functions intended to identify and remove potential
deadheads. We'll begin exploring these in the following section.
...
Let's explore pings 16770 through 16910 from trip 1306100. We can
...
sense for your data. Read more about these options and the theory
behind median filters at `help(clean_jumps)`.
...
alignment. Let's take a look at a the points removed:
...
`return_changes`. Below we plot an example trip, 21499100, around one stop
it makes:
...
(`vignette("articles/intro-trajectories")`) we will fit and explore these
